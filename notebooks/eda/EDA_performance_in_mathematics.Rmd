...
+ The data set has a total of 395 observation and 31 predictors. Most of these I consider are better if are treated as factors. Only Age and Absences are treated as numeric.
...
```{r}
# Set names, exclude response variables
predictors_names <- set_names(names(clean_data)[-c(33:34)])
```
...
+ In terms of absences, there are 6 students with 30 absences or more.

```{r message=FALSE, warning=FALSE}
ggplot(clean_data, aes(x = absences)) +
          geom_histogram(stat = "count")  +
          theme_bw() +
          theme(axis.text=element_text(size = 21),
                axis.title=element_text(size = 21,face = "bold"),
...
plots_g3 <- map(predictors_names[-c(3,29,30,31,32)], ~ g3_plot(.x) ) 
...
```{r echo = FALSE}
print(paste0("Number of individuals with a score equal to 4: ",length(clean_data$g3[clean_data$g3 == 4])))
print(paste0("Number of individuals with a score equal to 20: ",length(clean_data$g3[clean_data$g3 == 20])))
```
...
g3_boxplot <-  function(x) {
    ggplot(clean_data, aes(y = g3, x = .data[[x]])) +
